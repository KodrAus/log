...
#[macro_export]
macro_rules! log {
    (target: $target:expr, $lvl:expr, msg: { $($arg:tt)+ }, kvs: { $($prop:tt)* }) => ({
        let lvl = $lvl;
    ...
    ($lvl:expr, msg: { $($arg:tt)+ }, kvs: { $($prop:tt)* }) => (log!(target: module_path!(), $lvl, msg: { $($arg)+ }, kvs: { $($prop)* }));

    (target: $target:expr, $lvl:expr, $($arg:tt)+) => (log!(target: $target, $lvl, msg: { $($arg)+ }, kvs: {}));
